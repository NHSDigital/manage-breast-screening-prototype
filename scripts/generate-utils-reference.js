...
const FILE_META = {
  ...
  'app/lib/utils/appointment-status.js': {
    ...
    description:
      'The appointment status-change funnel. Changing a status also moves the appointment’s episode along, so the two updates live together here.'
  },
...
function getExports(source) {
  ...
  if (match) {
    return (match[1].match(/\b([a-zA-Z_$][\w$]*)\b/g) || []).filter(
      (name) => !['true', 'false', 'null', 'undefined'].includes(name)
    )
  }
...
function extractDocumentedFunctions(source) {
  ...
  while ((match = pattern.exec(source)) !== null) {
    ...
    if (/=\s*\{/.test(declarationLine) && !/=>|function/.test(declarationLine))
      continue
    ...
    if (params.length === 0) {
      const sigMatch = declarationLine.match(
        /=\s*(?:async\s+)?\(([^)]*)\)|(?:async\s+)?function\s+\w+\s*\(([^)]*)/
      )
      if (sigMatch) {
...
function generateFileSection(filePath) {
  ...
  if (functions.length === 0) {
    console.warn(
      `  Warning: no documented exported functions found in ${filePath}`
    )
    return ''
  }
...
function generate() {
  ...
  const utilsSections = UTILS_FILES.map((file) => {
    ...
    const section = {
      meta: FILE_META[file],
      content: generateFileSection(file)
    }
    console.log('done')
    return section
  })
  ...
  const tocLineCount =
    2 + 1 + 1 + UTILS_FILES.length + 1 + FILTER_FILES.length + 3

  let cursor =
    1 + countNewlines(header) + tocLineCount + countNewlines(utilsIntro)
  ...
  if (actualTocLineCount !== tocLineCount) {
    console.warn(
      `Warning: ToC line count mismatch (expected ${tocLineCount}, got ${actualTocLineCount}). Line numbers may be off by ${actualTocLineCount - tocLineCount}.`
    )
  }
