...
- **Read submitted values from `req.session.data`, not `req.body`.** By the time a route runs, the body has already been merged into the session. Reading `req.body` bypasses the session and breaks the auto-store behaviour everything else relies on. There are two exceptions. JSON endpoints called from client-side JavaScript must use `req.body`, because JSON is parsed after the auto-store runs. One-off action parameters, such as a submit button's value or the hidden ids and optional flags on a fragment action, are better read from `req.body` too (or `req.query` when a link sends them): in the session they stay after the request, so a later request that leaves them out would pick up an old value. Starting such a field's name with an underscore (`_exitAction`) keeps it out of the session altogether, because the kit skips those.
