...
- `/review/issues` - the index, scoped to the current user's BSU, with a Home back link. Open, Resolved and All are tabs with counts that follow the search and filters, open by default. The filter panel offers issue type, episode stage (where the round is now), "Raised by me" and a name or NHS number search (`getIssueFilterGroups`), and results sort by raised date or surname (`ISSUE_SORTS`). Each row leads with the participant's name in bold, then the type and description, and ends in a "View issue" link
- `/review/issues/:issueId` - the issue: an "Issue details" feature card (description, who raised it, when and where, reference) with a Change link to `/review/issues/:issueId/description` while open (modal-capable, description required), a Participant card per linked participant (their details, "View …" links to the linked episode, appointment and case, and any other open issues they have), and the resolve form (outcome required, note optional), which posts to `/review/issues/:issueId/resolve`. The page, the description edit and resolve are not found for an issue outside the user's BSU (`isIssueInUnit`)

## Form answers keyed to their record

The raise and resolve forms keep their answers in session data with the id of the record they were given for: `data.raiseIssue.recordId`, `data.issueResolution.issueId` and `data.issueDescription.issueId`. A route only reuses answers for the same record, so opening another issue or case starts a fresh form, and a validation error still keeps what was entered. The routes pass the answers to the template as locals (`answers`, `resolutionAnswers`, `descriptionAnswers`), because the template's `data` is a copy taken before the route runs, and delete them once the issue is created or resolved, since the kit copies every posted field into the session.
