...
- `/review/issues` - the index, scoped to the current user's BSU, with a Home back link. Open, Resolved and All are tabs with counts that follow the search and filters, open by default. The filter panel offers issue type, episode stage (where the round is now), raised by (with the single option "Me") and a name or NHS number search (`getIssueFilterGroups`), and results sort by raised date or surname (`ISSUE_SORTS`). Each row leads with the participant's name in bold, then the type and description, and ends in a "View issue" link
- `/review/issues/:issueId` - the issue: an "Issue details" feature card (description, who raised it, when and where, reference) with a Change link to `/review/issues/:issueId/description` while open (modal-capable, description required), a Participant card per linked participant (their details, "View …" links to the linked episode, appointment and case, and any other open issues they have), and the resolve form (outcome required, note optional), which posts to `/review/issues/:issueId/resolve`. The page, the description edit and resolve render the kit's page not found for a missing issue or one outside the user's BSU (`isIssueInUnit`)
