...
;(function () {
  ...
  function getImageRect(panel) {
    ...
    var isLeft =
      panel.classList.contains('app-annotation-images__panel--left') ||
  ...
  function containerFractionToImageFraction(cx, cy, panel) {
    ...
    return {
      x: Math.max(
        0,
        Math.min(1, (cx * rect.panelW - rect.offsetX) / rect.renderedW)
      ),
      y: Math.max(
        0,
        Math.min(1, (cy * rect.panelH - rect.offsetY) / rect.renderedH)
      )
    }
  }
  ...
  function init(options) {
    ...
    // State
    var annotations = (opts.annotations || []).map(function (a) {
    ...
    function getPanelKeysForSide(side) {
      var panels = container.querySelectorAll(
        '[data-panel-side="' +
          side +
          '"] .app-annotation-images__panel[data-view]'
      )
      var keys = []
    ...
    function renderMarkers() {
      ...
      container
        .querySelectorAll(
          '[data-panel-side="' + activeSide + '"] .app-ann-marker'
        )
        .forEach(function (m) {
          m.remove()
        })
      ...
      sideAnnotations.forEach(function (ann) {
        ...
        Object.keys(ann.positions).forEach(function (viewKey) {
          ...
          var panelEl = container.querySelector(
            '[data-panel-side="' +
              activeSide +
              '"] [data-view="' +
              viewKey +
    ...
    function attachMarkerHandlers(marker) {
      marker.addEventListener('click', function (e) {
        ...
        if (
          targetAnn &&
          !(targetAnn.abnormalityTypes && targetAnn.abnormalityTypes.length) &&
          !targetAnn.levelOfConcern
        ) {
          var panelKeys = getPanelKeysForSide(targetAnn.side)
          if (
            panelKeys.length > 0 &&
            panelKeys.every(function (k) {
              return targetAnn.positions && targetAnn.positions[k]
            })
          ) {
      ...
      marker.addEventListener('mousedown', function (e) {
        ...
        if (activeAnnotationId !== annId) {
          ...
          container
            .querySelectorAll('.app-ann-marker.is-active')
            .forEach(function (m) {
              m.classList.remove('is-active')
            })
          container
            .querySelectorAll('.app-ann-marker[data-ann-id="' + annId + '"]')
            .forEach(function (m) {
        ...
        function onUp(upEvent) {
          ...
          if (hasDragged) {
            ...
            if (panel) {
              ...
              var cy = Math.max(
                0,
                Math.min(1, (upEvent.clientY - rect.top) / rect.height)
              )
              var imagePos = containerFractionToImageFraction(cx, cy, panel)
              var ann = getAnnotation(annId)
              if (ann) {
                if (!ann.positions) ann.positions = {}
                ann.positions[viewKey] = {
                  x: Math.round(imagePos.x * 1000) / 1000,
                  y: Math.round(imagePos.y * 1000) / 1000
                }
              }
            }
            autoSave()
    ...
    function renderSidebar() {
      ...
      var sidebarScroll = container.querySelector(
        '[data-panel-side="' + activeSide + '"] .app-ann-v2__sidebar-scroll'
      )
      ...
      sideAnnotations.forEach(function (ann) {
        ...
        if (ann.abnormalityTypes && ann.abnormalityTypes.length) {
          typeLabel = Array.isArray(ann.abnormalityTypes)
            ? ann.abnormalityTypes.join(', ')
            : ann.abnormalityTypes
        }
        var concernLabel = ann.levelOfConcern
          ? CONCERN_LABELS[ann.levelOfConcern]
          : ''
        ...
        html +=
          ...
          '<span class="app-annotation-list__number">' +
          (displayIndex[ann.id] || '?') +
          '</span>' +
          '<div class="app-annotation-list__body">' +
          '<span class="app-annotation-list__type">' +
          typeLabel +
          '</span>'

        if (concernLabel) {
          html +=
            '<p class="app-annotation-list__meta">' + concernLabel + '</p>'
        }
        if (ann.comment) {
        ...
        if (isIncomplete) {
          html +=
            ...
            '<button class="nhsuk-button nhsuk-button--small nhsuk-u-margin-bottom-0" type="button" data-action="edit" data-ann-id="' +
            ann.id +
            '">Add details</button>' +
            ' <button class="app-link-button app-link-button--warning" type="button" data-action="delete-annotation" data-ann-id="' +
            ann.id +
            '">Remove</button>' +
            '</div>'
        } else {
      ...
      var addBtnText =
        sideAnnotations.length > 0 ? 'Add another annotation' : 'Add annotation'
    ...
    function renderTabs() {
      ...
      ;['right', 'left'].forEach(function (side) {
        var countSpan = document.querySelector(
          '[data-annotation-count="' + side + '"]'
        )
        if (countSpan) {
          countSpan.dataset.count = annotations.filter(function (a) {
            return a.side === side
          }).length
        }
      })
    ...
    function updateCursorPreview(panel, event) {
      if (
        event.target.closest('.app-ann-marker') ||
      ...
      if (cursorPreview.parentElement !== panel)
        panel.appendChild(cursorPreview)
      ...
      if (incompleteAnn) {
        ...
        var existingMarker = panel.querySelector(
          '.app-ann-marker[data-ann-id="' + incompleteAnn.id + '"]'
        )
      ...
      if (
        ann &&
        ann.side === activeSide &&
        (!ann.positions || !ann.positions[viewKey])
      ) {
    ...
    function handleImageClick(event) {
      ...
      if (incompleteAnn) {
        ...
        var allCovered =
          panelKeys.length > 0 &&
          panelKeys.every(function (k) {
            return incompleteAnn.positions[k]
          })
      ...
      var allCovered2 =
        panelKeys2.length > 0 &&
        newAnn &&
        panelKeys2.every(function (k) {
          return newAnn.positions[k]
        })
    ...
    function openModal(annId) {
      ...
      document.getElementById('ann-modal-title').textContent = isIncomplete
        ? 'Add annotation'
        : 'Update annotation'
      document.getElementById('ann-modal-save').textContent = isIncomplete
        ? 'Add annotation'
        : 'Update annotation'

      // Set abnormality types
      var currentTypes =
        ann && ann.abnormalityTypes
          ? Array.isArray(ann.abnormalityTypes)
            ? ann.abnormalityTypes
            : [ann.abnormalityTypes]
          : []
      var abTypesEl = document.querySelector(
      ...
      document
        ...
        .forEach(function (radio) {
          radio.checked =
            radio.value ===
            (ann && ann.levelOfConcern ? ann.levelOfConcern : '')
        })
    ...
    function saveModal() {
      ...
      if (abTypesEl && abTypesEl.tagName === 'SELECT') {
        ...
        savedTypes = Array.from(
          document.querySelectorAll(
            '[name="imageReadingTemp[annotationTemp][abnormalityTypes]"]:checked'
          )
        ).map(function (cb) {
          return cb.value
        })
    ...
    function openLightbox(sourcePanel) {
      ...
      var isLeft = sourcePanel.classList.contains(
        'app-annotation-images__panel--left'
      )

      lightboxPanel.innerHTML = ''
      lightboxPanel.className =
        'app-ann-v2__lightbox-panel app-ann-v2__image-panel app-ann-v2__image-panel--' +
        (isLeft ? 'left' : 'right')
    ...
    function renderLightboxMarkers() {
      ...
      lightboxPanel.querySelectorAll('.app-ann-marker').forEach(function (m) {
        m.remove()
      })
        ...
        marker.addEventListener('mousedown', function (e) {
          ...
          function onUp(upEvent) {
            ...
            if (hasDragged) {
              ...
              var imgPos = containerFractionToImageFraction(
                cx,
                cy,
                lightboxPanel
              )
              ...
              if (dragAnn) {
                ...
                dragAnn.positions[lightboxViewKey] = {
                  x: Math.round(imgPos.x * 1000) / 1000,
                  y: Math.round(imgPos.y * 1000) / 1000
                }
              }
              renderMarkers()
              renderSidebar()
    ...
    function onLightboxMouseMove(e) {
      ...
      if (lightboxCursorPreview.parentElement !== lightboxPanel)
        lightboxPanel.appendChild(lightboxCursorPreview)
      var rect = lightboxPanel.getBoundingClientRect()
      ...
      if (incompleteAnn) {
        ...
        if (
          ann &&
          ann.side === activeSide &&
          (!ann.positions || !ann.positions[lightboxViewKey])
        ) {
    ...
    function onLightboxClick(e) {
      ...
      setTimeout(function () {
        ...
        if (
          updated &&
          !(updated.abnormalityTypes && updated.abnormalityTypes.length) &&
          !updated.levelOfConcern
        ) {
          var panelKeys = getPanelKeysForSide(activeSide)
          if (
            panelKeys.length > 0 &&
            panelKeys.every(function (k) {
              return updated.positions && updated.positions[k]
            })
          ) {
    ...
    if (lightbox) {
      document
        .getElementById('ann-lightbox-close')
        .addEventListener('click', closeLightbox)
      document
        .getElementById('ann-lightbox-overlay')
        .addEventListener('click', closeLightbox)
    ...
    if (annModal) {
      document
        .getElementById('ann-modal-save')
        .addEventListener('click', saveModal)
      document
        .getElementById('ann-modal-cancel')
        .addEventListener('click', cancelModal)
      document
        .getElementById('ann-modal-delete')
        .addEventListener('click', deleteModalAnnotation)
      document
        .getElementById('ann-modal-close-x')
        .addEventListener('click', cancelModal)
      annModal
        .querySelector('.app-modal__overlay')
        .addEventListener('click', cancelModal)
    ...
    container.querySelectorAll('.app-ann-marker').forEach(function (m) {
      m.remove()
    })
  ...
  function correctContainer(cont) {
    ...
    panels.forEach(function (panel) {
      panel.querySelectorAll('.app-ann-marker').forEach(function (marker) {
        ...
        var corrected = imageFractionToContainerPercent(
          { x: rawX, y: rawY },
          panel
        )
