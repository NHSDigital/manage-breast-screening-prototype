...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const participantName = document
    .querySelector('meta[name="participant-name"]')
    ?.getAttribute('content')

  const eventId = document
    .querySelector('meta[name="event-id"]')
    ?.getAttribute('content')
  ...
  updateExisting: function (participantName) {
    ...
    try {
      ...
      if (!updatedSuccessfully) {
        ...
        const nameElement =
          mammogramWindow.document.getElementById('participant-name')
        if (nameElement) {
          ...
          if (typeof mammogramWindow.randomizeAllImages === 'function') {
            ...
            const nameSeed = participantName
              .split('')
              .reduce((sum, char, i) => sum + char.charCodeAt(0) * (i + 1), 0)

            mammogramWindow.Math.random =
              mammogramWindow.Math.seedrandom(nameSeed)
...
window.addEventListener('popstate', function () {
  ...
  if (
    window.inReadingContext &&
    !window.location.pathname.includes('/reading/batch/')
  ) {
