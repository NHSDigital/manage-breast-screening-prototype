...
const generatePregnancyAndBreastfeeding = (options = {}) => {
  ...
  if (info.pregnancyStatus === 'yes') {
    info.currentlyPregnantDetails =
      faker.helpers.arrayElement(PREGNANCY_DETAILS)
  } else if (info.pregnancyStatus === 'noButRecently') {
    info.recentlyPregnantDetails = faker.helpers.arrayElement(
      RECENT_PREGNANCY_DETAILS
    )
  }
  ...
  if (info.breastfeedingStatus === 'yes') {
    info.currentlyBreastfeedingDuration = faker.helpers.arrayElement(
      BREASTFEEDING_DURATIONS
    )
  } else if (info.breastfeedingStatus === 'recentlyStopped') {
    info.recentlyBreastfeedingDuration = faker.helpers.arrayElement(
      RECENTLY_STOPPED_BREASTFEEDING
    )
  }
