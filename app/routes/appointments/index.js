...
} = require('../../lib/utils/dynamic-routing')
...
module.exports = (router) => {
  ...
  router.use(
    ...
    (req, res, next) => {
      ...
      const originalAppointmentData = getAppointmentData(
        req.session.data,
        clinicId,
        appointmentId
      )
      ...
      if (!originalAppointmentData) {
        console.log(
          `No appointment ${appointmentId} found for clinic ${clinicId}`
        )
      ...
      const savedAppointment = data.appointments.find(
        (appointment) => appointment.id === data.appointment.id
      )
      const tempAppointmentForCompare = data.appointment
        ? { ...data.appointment }
        : undefined
      const savedAppointmentForCompare = savedAppointment
        ? { ...savedAppointment }
        : undefined
      if (tempAppointmentForCompare)
        delete tempAppointmentForCompare.workflowStatus
      if (savedAppointmentForCompare)
        delete savedAppointmentForCompare.workflowStatus
